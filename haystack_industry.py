...
from haystack_utilities import (
    PROCESSED_FOLDER, INDUSTRY_FOLDER, ANALYSIS_FOLDER, 
    pd, makedir
)
...
class Industry:
    ...
    def __init__(self, ticker=None, filename=None,
                 from_folder=(f"{PROCESSED_FOLDER}{ANALYSIS_FOLDER}"
                              f"{INDUSTRY_FOLDER}"),
                 to_folder=(f'{PROCESSED_FOLDER}{INDUSTRY_FOLDER}'
                             'score_total_reports/')):

        if ticker is not None or filename is not None:
            analyst = Analyst(ticker=ticker, filename=filename,
                              from_folder=from_folder)
            self.ticker = analyst.ticker
            self.folder = f'{to_folder}{self.ticker}'
            makedir(self.folder)
            try:
                self.data = pd.read_csv(analyst.filename, 
                                          header=0,
                                          index_col=0,
                                          engine='c')
            ...
            if self.data is not None:
                ...
                df['SCORE_PRICE'] = (
                    df.AVERAGE_PRICE_RATIOS/ 
                    max(df.AVERAGE_PRICE_RATIOS)
                )
                df["SCORE_RETURNS"] = (
...
SCORE
{str(df.COMPANY)} ranks Number {rank} on the {category.replace('_', ' ')} rankings in the {str(df.SECTOR)} Industry with a Growth Score of {df.SCORE_GROWTH:.4f}, Price Score of {df.SCORE_PRICE:.4f}, Returns Score of {df.SCORE_RETURNS:.6f} and Safety Score of {df.SCORE_SAFETY:.4f} for a Total Score of {df.SCORE_TOTAL:.4f}."""
