...
from haystack_utilities import PROCESSED_FOLDER, SECTORS_FOLDER, ANALYSIS_FOLDER, pd


class Sectors:

    def __init__(self, 
                 sectors_folder=None, stocks_folder=None,
                 to_folder=(f'{PROCESSED_FOLDER}{ANALYSIS_FOLDER}'
                            f'{SECTORS_FOLDER}'),
                 ):
        analyst = Analyst(from_folder=sectors_folder,
                          to_folder=to_folder)
        ...
        if self.stocks_df is not None and self.sectors_df is not None:
            ...
            self.symbols["AVERAGE_PRICE_RATIOS"] = pd.np.mean([
