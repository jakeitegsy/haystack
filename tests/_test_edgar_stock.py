...
analyst = Stock(source='EDGAR', ticker='A').get_stock()
...
class TestEdgar(unittest.TestCase):
    ...
    def test_get_net_values(self):
        assert_index_equal(
            analyst.get_net_values(raw_data).columns,
            Index([
                'CURRENT_ASSETS',
                'CURRENT_LIABILITIES',
                'NET_ASSETS',
    ...
    def test_moving_averages_keep_same_columns_as_net_calculations(self):
        assert_index_equal(
            analyst.get_moving_averages(raw_data).columns,
            analyst.get_net_values(raw_data).columns
        )

    def test_get_moving_average_differences(self):
        assert_index_equal(
            analyst.get_moving_average_differences(raw_data).columns,
            Index([
                'DIFF_ASSETS_DEBT',
    ...
    def test_get_growth_rates(self):
        assert_index_equal(
            analyst.get_moving_average_growth_rates(raw_data).index,
            Index([
    ...
    def test_get_moving_average_ratios(self):
        assert_index_equal(
            analyst.get_moving_average_ratios(raw_data).columns,
            Index([
                'RATIO_CASH_ASSETS',
                'RATIO_CASH_DEBT',
    ...
    def test_get_moving_average_sums(self):
        assert_index_equal(
            analyst.get_moving_average_sums(raw_data).index,
            analyst.get_moving_averages(raw_data).columns
        )

    def test_get_average_per_share_averages(self):
        assert_index_equal(
            analyst.get_average_per_share_averages(raw_data).index,
            analyst.get_moving_averages_per_share(raw_data).columns
        )

    def test_average_per_share_differences(self):
        assert_index_equal(
            analyst.get_average_per_share_differences(raw_data).index,
            Index([
    ...
    def test_get_average_moving_averages(self):
        assert_index_equal(
            analyst.get_average_moving_averages(raw_data).index,
            analyst.get_moving_averages(raw_data).columns
        )
    ...
    def test_average_moving_average_differences(self):
        assert_index_equal(
            analyst.get_moving_average_differences(raw_data).columns,
            analyst.get_average_moving_average_differences(raw_data).index
        )

    def test_average_moving_average_ratios(self):
        assert_index_equal(
            analyst.get_average_moving_average_ratios(raw_data).index,
            analyst.get_moving_average_ratios(raw_data).columns
        )
    ...
    def test_get_median_growth_rate(self):
        growth_rates = analyst.get_moving_average_growth_rates(raw_data)
        self.assertEqual(
            analyst.get_median_growth_rate(raw_data),
            median([
    ...
    def test_get_median_returns(self):
        ratios = analyst.get_moving_average_ratios(raw_data)
        self.assertEqual(
            analyst.get_median_returns(raw_data),
            median([
    ...
    def test_get_median_safety(self):
        safety = analyst.get_average_moving_average_differences(raw_data)
        self.assertEqual(
            analyst.get_median_safety(raw_data),
            median([
                ...
                analyst.get_average_moving_averages(raw_data)['NET_WORKING_CAPITAL'],
            ])
        )

    def test_get_averages(self):
        assert_index_equal(
            analyst.get_averages(raw_data).index,
            Index([
                'AVERAGE_GROWTH',
                'AVERAGE_RETURNS',
                'AVERAGE_SAFETY',
    ...
    def test_stock_summary(self):
        assert_index_equal(
            ...
            concat([
                analyst.get_averages(raw_data),
                analyst.get_average_moving_average_differences(raw_data),
                analyst.get_moving_average_growth_rates(raw_data),
                analyst.get_average_moving_averages(raw_data),
                analyst.get_average_per_share_averages(raw_data),
                analyst.get_average_per_share_differences(raw_data),
                analyst.get_average_moving_average_ratios(raw_data),
            ], axis=0).index
        )