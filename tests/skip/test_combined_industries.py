...
industry_folder = f"{folder}{INDUSTRY_FOLDER}"
edgar_folder = f"{folder}{EDGAR_FOLDER}"
stockpup_folder = f"{folder}{STOCKPUP_FOLDER}"
edgar_out = f"{industry_folder}{EDGAR_FOLDER}"
stockpup_out = f"{industry_folder}{STOCKPUP_FOLDER}"


def test_prep(industry_folder=None, stocks_folder=None, to_folder=None):
    industries = Industries(
        industry_folder=industry_folder,
        stocks_folder=stocks_folder,
        to_folder=to_folder,
    )
    while industries.stocks_df is None:
        test_prep(industries_folder=industries_folder, stocks_folder=stocks_folder,
                  to_folder=to_folder)

    return industries


edgar_industries = test_prep(industry_folder=industry_folder,
                          stocks_folder=edgar_folder,
                          to_folder=edgar_out)
stockpup_industries = test_prep(industry_folder=industry_folder,
...
class TestDataFrames(unittest.TestCase):
    ...
    def test_industries_df_has_3_columns(self):
        self.assertEqual(len(edgar_industries.industries_df.columns), 3)
        self.assertEqual(
            len(stockpup_industries.industries_df.columns), 3
        )
        self.assertEqual(
            sorted(edgar_industries.industries_df.columns),
            sorted(stockpup_industries.industries_df.columns)
        )
        self.assertEqual(
            sorted(edgar_industries.industries_df.columns), 
    ...
    def test_stocks_df_has_146_columns(self):
        self.assertEqual(len(edgar_industries.stocks_df.columns), 146)
        self.assertEqual(len(stockpup_industries.stocks_df.columns), 146)
        self.assertEqual(
            sorted(edgar_industries.stocks_df.columns),
            sorted(stockpup_industries.stocks_df.columns)
        )
...
class TestPrices(unittest.TestCase):
    ...
    def setUp(self):
        self.edgar_prices = edgar_industries.symbols.PER_SHARE_MARKET
    ...
    def test_prices_is_a_series_with_the_same_index_as_combined_df(self):
        self.assertEqual(
            ...
            sorted(edgar_industries.symbols.index)
        )
        self.assertEqual(
            sorted(self.stockpup_prices.index),
            sorted(stockpup_industries.symbols.index)
        )
...
class TestPriceRatios(unittest.TestCase):
    ...
    def test_price_ratios_columns_are_added_to_combined_df(self):
        [self.assertIn(name, sorted(edgar_industries.symbols.columns))
            for name in self.comparisons
        ]
        [self.assertIn(name, 
            sorted(stockpup_industries.symbols.columns))
            for name in self.comparisons
        ]
...
class TestReports(unittest.TestCase):
    ...
    def setUp(self):
        self.maxDiff = None
        self.industries = [
            "basic", "capital_goods", "consumer_durables", 
            "consumer_nondurables", "consumer_services", "finance",
            "misc", "technology", "transportation", "unknown", 
            "utilities",
        ]

    def test_combined_report_has_161_columns(self):
        self.assertEqual(len(edgar_industries.symbols.columns), 161)
        self.assertEqual(len(stockpup_industries.symbols.columns), 161)
        self.assertEqual(
            sorted(edgar_industries.symbols.columns),
            sorted(stockpup_industries.symbols.columns)
        )
        self.assertEqual(
            sorted(edgar_industries.symbols.columns),
    ...
    def test_tickers_with_no_price_data_are_dropped(self):
        self.assertEqual(
            len(edgar_industries.symbols[
                edgar_industries.symbols.PER_SHARE_MARKET == 0
            ]), 0
        )
        self.assertEqual(
            len(stockpup_industries.symbols[
                stockpup_industries.symbols.PER_SHARE_MARKET == 0
            ]), 0
        )

    def test_reports_contains_groups_with_sector_symbol(self):
        self.assertEqual(
            sorted(list(edgar_industries.reports.groups.keys())),
            sorted(list(stockpup_industries.reports.groups.keys()))
        )
        self.assertEqual(
            sorted(list(edgar_industries.reports.groups.keys())),
    ...
    def test_write_reports_sector_files(self):
        ...
        self.assertEqual(
            sorted(list_filetype(in_folder=edgar_out)),
            sorted([sector + ".csv" for sector in self.industries])
        )
